...
impl<'d> PwmChannel<'d> {
    ...
    pub fn configure(&mut self, freq: u32, duty_percent: u8) {
        ...
        r.pwm_freq_l0().write(|w| unsafe { w.bits((period & 0xFFFF) as u32) });
        r.pwm_freq_h0().write(|w| unsafe { w.bits(((period >> 16) & 0xFFFF) as u32) });
        r.pwm_duty_l0().write(|w| unsafe { w.bits((duty & 0xFFFF) as u32) });
        r.pwm_duty_h0().write(|w| unsafe { w.bits(((duty >> 16) & 0xFFFF) as u32) });
    }

    pub fn enable(&mut self) { self.regs().pwm_en0().write(|w| unsafe { w.bits(1u32) }); }
    pub fn disable(&mut self) { self.regs().pwm_en0().write(|w| unsafe { w.bits(0u32) }); }
    ...
    pub fn start(&mut self) {
        self.regs().pwm_start0().write(|w| unsafe { w.bits((1u32 << self.channel) as u32) });
    }
